...
Latest release: 0.2.10 - MariaDB 10.1.11 - [Changelog](CHANGELOG.md) | [Docker Hub](https://hub.docker.com/r/osixia/mariadb/) 
...
- [Advanced User Guide](#advanced-user-guide)
	- [Extend osixia/mariadb:0.2.10 image](#extend-osixiamariadb0210-image)
...
Run MariaDB docker image:
	...
	docker run --name my-mariadb-container --detach osixia/mariadb:0.2.10
...
By default SSL is enable, a certificate is created with the container hostname (it can be set by docker run --hostname option eg: phpldapadmin.my-company.com).
	...
	docker run --hostname db.my-company.com --detach osixia/mariadb:0.2.10
...
Add --env MARIADB_SSL=false to the run command :
    ...
    docker run --env MARIADB_SSL=false --detach osixia/mariadb:0.2.10
...
To fix that run the container with `--copy-service` argument :
		...
		docker run [your options] osixia/mariadb:0.2.10 --copy-service
...
Example command to run the container in `debug` mode:
	...
	docker run --detach osixia/mariadb:0.2.10 --loglevel debug

See all command line options:

	docker run osixia/mariadb:0.2.10 --help
...
- **MARIADB_ROOT_ALLOWED_NETWORKS**: root login will only be allowed from those networks. Defaults to:
  ...
  - ::1
	...
	If you want to set this variable at docker run command add the tag `#PYTHON2BASH:` and convert the yaml in python:
		...
		docker run --env PHPLDAPADMIN_LDAP_HOSTS="#PYTHON2BASH:['localhost','127.0.0.1','::1']" --detach osixia/mariadb:0.2.10
...
Environment variables can be set by adding the --env argument in the command line, for example:
	...
	docker run --env MARIADB_ROOT_USER="JaxTeller" --env MARIADB_ROOT_PASSWORD="Sons Of Anarchy" --detach osixia/mariadb:0.2.10
...
### Extend osixia/mariadb:0.2.10 image

If you need to add your custom TLS certificate, bootstrap config or environment files the easiest way is to extends this image.

Dockerfile example:

    FROM osixia/mariadb:0.2.10
